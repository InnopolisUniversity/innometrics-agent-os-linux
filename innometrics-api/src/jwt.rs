//! Handle JWT authentication for Innometrics.
...
impl Claims {
    ...
    /// Check whether the token is going to expire soon enough to refresh it already.
    ///
    /// By default the deadline is 10 minutes prior to expiration date.
    ///
    /// Returns true if token has to be refreshed before making new requests.
    pub fn expires_soon(&self) -> bool {
        let leeway = Duration::minutes(10);
        let deadline = self.exp - leeway;
        let now = Utc::now();
        now > deadline
    }
}
...
mod tests {
    ...
    fn test_refresh_deadline() {
        let token = "eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJpLnRrYWNoZW5rb0Bpbm5vcG9saXMucnUiLCJleHAiOjE1OTM0NDMwNTgsImlhdCI6MTU5MDg1MTA1OH0.BkzVcl4U5fLS_5HPBomhYkH3iyo-mfbQ-0Sco96D_FUdEDUbrzl8t97rcsUvIVs_rsMG66DMqjUvzVQ0HrE29w";
        let token_data = jsonwebtoken::dangerous_unsafe_decode::<Claims>(token).unwrap();

        let exp = token_data.claims.exp.clone();
        let now = "2020-05-30T20:39:53Z".parse::<DateTime<Utc>>().unwrap();
        let deadline = exp - Duration::minutes(10);
        let is_time_to_refresh = now > deadline;
        assert!(!is_time_to_refresh);
    }
}
